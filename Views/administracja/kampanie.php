...
<div class="d-flex justify-content-between align-items-baseline mb-4">
  ...
  <div class="d-flex gap-2">
    ...
    <span class="text-muted small align-self-center">Szablony: <code>public/maile/</code></span>
  </div>
</div>
        ...
        <div class="d-flex flex-column gap-2">
          <div class="form-check">
            ...
            <label class="form-check-label" for="tgActive">
              Gracze aktywnego turnieju <span class="badge bg-secondary"><?= (int)$activeCount ?></span>
            </label>
          </div>
          <div class="form-check">
            <input class="form-check-input" type="radio" name="target_group" value="all" id="tgAll">
            <label class="form-check-label" for="tgAll">
              Wszyscy zarejestrowani <span class="badge bg-secondary"><?= (int)$allCount ?></span>
            </label>
          </div>
          <div class="form-check">
...
foreach ($sentMap as $tpl => $groups) {
  foreach ($groups as $group => $row) { $history[] = $row; }
}
usort($history, fn($a,$b) => strcmp((string)$b['sent_at'], (string)$a['sent_at']));
$groupLabels = ['active' => 'Aktywny turniej', 'all' => 'Wszyscy'];
?>
<?php if (!empty($history)): ?>
<div class="card border-0 shadow-sm">
  <div class="card-header fw-semibold">Historia wysyłek</div>
  <div class="card-body p-0">
    <table class="table table-sm table-hover mb-0">
      <thead class="table-light">
        <tr>
          <th class="ps-3">Szablon</th><th>Temat</th><th>Odbiorcy</th><th>Wysłano</th>
      ...
      <tbody>
        ...
        <tr>
          ...
          <td class="pe-3 text-end"><span class="badge bg-success"><?= (int)($h['recipients_count'] ?? 0) ?> os.</span></td>
        </tr>
        <?php endforeach ?>
      </tbody>
    </table>
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
        <div class="bg-light rounded p-3 small">
          <div class="row mb-1"><div class="col-4 text-muted">Szablon</div><div class="col-8 fw-semibold" id="mTemplate"></div></div>
          <div class="row mb-1"><div class="col-4 text-muted">Temat</div><div class="col-8" id="mSubject"></div></div>
          <div class="row"><div class="col-4 text-muted">Odbiorcy</div><div class="col-8" id="mTarget"></div></div>
        </div>
      ...
      <div class="modal-footer border-0 pt-0">
        ...
        <button type="button" class="btn btn-sm btn-primary" onclick="submitSend()">Wyślij →</button>
      </div>
    </div>
  </div>
</div>
...
document.getElementById('selectTemplate').addEventListener('change', function () {
  ...
  if (tpl && sentMap[tpl]) {
    const groups = Object.keys(sentMap[tpl]).filter(g => sentMap[tpl][g].sent_at).map(g => groupLabels[g] || g);
    status.innerHTML = groups.length ? '<span class="text-warning small">⚠ Wysłano już do: ' + groups.join(', ') + '</span>' : '';
  } else { status.innerHTML = ''; }
});

document.querySelectorAll('input[name="target_group"]').forEach(r => {
  r.addEventListener('change', function () {
    if (this.value === 'tournament') document.getElementById('inputTournamentId').focus();
  });
});
